/*
 * shamir.js — Feldman-verifiable secret sharing helpers (API-side copy)
 *
 * Shares are still generated with Shamir secret sharing, but over a large
 * prime field so we can attach Feldman commitments and verify each revealed
 * share before reconstruction.
 */
...
function shareStringToValue(data) {
  if (!/^[A-Za-z0-9_-]+$/u.test(data)) {
    throw new Error('Share data must be base64url-encoded');
  }
...
function splitSecretWithCommitments(secret, n, t) {
  if (t > n) throw new Error('Threshold must be ≤ share count');
