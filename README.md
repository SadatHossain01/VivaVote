...
The latest camera-ready evaluation uses a live two-organization Hyperledger Fabric 2.5 deployment and a naïve per-voter-registration baseline. Reported points are means over three trials.

| Metric                                     | VivaVote            | Baseline                 |
| ------------------------------------------ | ------------------- | ------------------------ |
| Election setup, 10–1000 voters             | ≈ **2.9–3.4 s**     | ≈ 6.0–6.7 s              |
| Eligibility state at 10,000 voters         | **2 keys / 1.9 kB** | 10,002 keys / 719 kB     |
| Commit throughput at 10 concurrent clients | **15.38 TPS**       | 15.77 TPS                |
| Accepted commits / MVCC conflicts          | **100% / 0**        | 100% / 0                 |
| Tally completeness at 50–100% reveal rates | **100%**            | Falls with voter dropout |

The Merkle-root design roughly halves setup time and keeps eligibility state constant. At 10 concurrent clients, its commit throughput is within 2.5% of the baseline while avoiding the shared counter hot keys that previously caused MVCC conflicts. With only 50% of voters returning to reveal, VivaVote still recovers and counts all 250 committed ballots; the baseline counts 125.
...
| Ledger   | Hyperledger Fabric 2.5, JavaScript chaincode                                          |
| Crypto   | Feldman-verifiable Shamir sharing over a 1024-bit prime field, RSA-OAEP-2048, SHA-256 |
...
| `JWT_SECRET`          | _(dev fallback in code)_ | JWT signing secret — override in production |
...
- **Ballot confidentiality boundary.** All commit-side cryptography runs in the voter's browser. Neither the API server nor ordinary ledger readers see a plaintext ballot. However, any colluding group of T or more trustees _can_ reconstruct individual ballots outside the normal protocol flow. VivaVote is not a coercion-resistant protocol — its confidentiality holds only below the trustee threshold.
